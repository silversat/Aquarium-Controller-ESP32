...
void Water_PH_Handler( float tmed ) {
	...
	if(phSetupEnded) {
		if((phTimer + PH_READ_INTERVAL) < millis()) {												//time interval: 1s
			voltagePH[ph_idx] = analogRead(PIN_PH_SENSOR) / ADC_RESOLUTION * PH_REF_VOLTAGE; 	// read the PH voltage (0~3.0V)
...
void Water_EC_Handler( float tmed ) {
	...
	if(ecSetupEnded) {
		if((ecTimer + EC_READ_INTERVAL) < millis()) {												//time interval: 1s
			voltageEC[ec_idx] = analogRead(PIN_EC_SENSOR) / ADC_RESOLUTION * EC_REF_VOLTAGE; 	// read the EC voltage (0~3.4V)
...
void PH_SensorInit() {
	int count;
	DEBUG("PH sensor (pin %d) values: ", PIN_PH_SENSOR);
	
	count = NvramReadAnything(NVRAM_PH_NEUTRAL, neutralVoltage);
	if(neutralVoltage <= 0 or neutralVoltage > 2000 or count != 4) {		// if value not set, write defaults to nvram
	...
	if(acidVoltage <= 0 or acidVoltage > 3000 or count != 4) {
		acidVoltage = 2032.44;
    ...
    ph.begin(neutralVoltage, acidVoltage);				// neutral = 1500.00, acid = 2032.44
...
void EC_SensorInit() {
	int count;
	DEBUG("EC sensor (pin %d) K values: ", PIN_EC_SENSOR);
	
	count = NvramReadAnything(NVRAM_EC_KVALUEHIGH, kvalueHigh);
	if(kvalueHigh <= 0.00 or kvalueHigh > 0xFF or count != 4) {
		kvalueHigh = 1.00;
	...
	if(kvalueLow <= 0.00 or kvalueLow > 0xFF or count != 4) {
		kvalueLow = 1.00;
