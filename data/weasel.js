	...
	if (WS && WS.readyState === 1 ){
		if ( typeof file === 'object' ){
			if( send_as_blob ){
				WS.binaryType = 'blob';
				WS.send(file);
				writeToScreen(`â™£ Blob Buffer Sent: ${file.name} [${file.size} B]`, 'outgoing');
			} else {
				...
				fileReader.onload = function (ev) {
					if (ev.target.readyState === FileReader.DONE){
						...
						writeToScreen(`â™  Array Buffer Sent: ${file.name} [${file.size} B]`, 'outgoing');
					}
				};
			}
		} else {
...
function switchAutoScroll(){
	...
	auto_scroll_marker.innerText = auto_scroll ? 'âš«' : 'âšª';
}
...
function writeBinaryLinks(link, bin_group, bin_position, extension){
	...
	var download = document.createElement('a');
		...
		download.href = link;
		download.innerText = 'SAVE â®Ÿ';
		links.appendChild(download);
	var open = document.createElement('a');
		open.target = '_blank';
		open.href = link;
		open.innerText = 'OPEN â¯';
		links.appendChild(open);
	var print = document.createElement('span');
		print.innerText = 'LOG â˜·';
